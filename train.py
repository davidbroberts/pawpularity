...
def fit(m, fold_n, training_batch_size = config['TRAIN_BATCH_SIZE'], validation_batch_size = config['VAL_BATCH_SIZE']):
    ...
    optimizer = SAM(m.parameters(), base_optimizer, lr=config['LR'] , weight_decay = 1e-6)
    
    wandb.watch(model, criterion, log="all", log_freq=10)
    
  
    num_train_steps = math.ceil(len(train_loader))
    num_warmup_steps = num_train_steps * (config['EPOCHS']//2 )
...
if __name__ == '__main__':
    ...
    train_aug = A.Compose(
        [
            ...
            A.Cutout(max_h_size=int(config['IMAGE_SIZE'] * 0.21), max_w_size=int(config['IMAGE_SIZE'] * 0.21), num_holes=5, p=0.5),
